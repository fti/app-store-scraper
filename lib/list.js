...
function cleanApp (app) {
  ...
  if (app['im:artist'].attributes) {
    ...
    if (app['im:artist'].attributes.href.includes('/id')) {
      ...
      developerId = app['im:artist'].attributes.href.split('/id')[1].split('?')[0];
    }
  }

  const price = parseFloat(app['im:price'].attributes.amount);
  let released = parseISO(app['im:releaseDate'].label);
  released = format(released, 'MMM dd, yyyy');
  return {
    id: app.id.attributes['im:id'],
    appId: app.id.attributes['im:id'],
    bundleId: app.id.attributes['im:bundleId'],
...
function list (opts) {
  return new Promise(function (resolve, reject) {
    ...
    const url = `https://itunes.apple.com/WebObjects/MZStoreServices.woa/ws/RSS/${opts.collection}/${category}/limit=${opts.num}/json?s=${storeId}`;
